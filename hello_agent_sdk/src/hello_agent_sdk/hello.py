...
async def on_chat_start():
    ...
    cl.user_session.set("history", [{"role": "assistant", "content": "Hello!How can I help you?"}])
# Step-5 : Run the agent
...
async def main(message: cl.Message):
    ...
    # msg = cl.Message(content=message.content) # to show user message immediately
    await msg.send()
    history.append({"role": "user", "content": message.content})
    ...
    async for chunk in result.stream_events():
        if chunk.type == "raw_response_event" and isinstance(chunk.data, ResponseTextDeltaEvent):
            token = chunk.data.delta
            await msg.stream_token(token)
    history.append({"role": "assistant", "content": result.final_output})
    cl.user_session.set("history", history)
    # await cl.Message(content=result.final_output).send() [after streaming the response, no need to send again]
    # print(result.final_output)
